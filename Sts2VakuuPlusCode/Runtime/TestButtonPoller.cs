...
internal sealed partial class TestButtonPoller : Node
{
    ...
    public static void Install(SceneTree tree)
    {
        try
        {
            var node = new TestButtonPoller { Name = "Sts2VakuuPlusTestButtonPoller" };
            tree.Root.CallDeferred(Node.MethodName.AddChild, node);
            MainFile.Logger.Info("[VakuuPlus] test-button poller installed");
        }
        catch (Exception ex)
        {
            MainFile.Logger.Warn($"[VakuuPlus] poller install failed: {ex.Message}");
        }
    }
    ...
    public override void _Process(double delta)
    {
        ...
        try
        {
            ...
            if (endTurn.GetNodeOrNull("VakuuPlusTestButton") is Button vakuuBtn)
            {
        ...
        {
            MainFile.Logger.Warn($"[VakuuPlus] poller tick failed: {ex.Message}");
        }
    }
