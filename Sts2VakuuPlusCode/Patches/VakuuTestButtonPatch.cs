...
using Playstyle = Sts2VakuuPlus.Planner.Playstyle;
...
{
    private const string ButtonName = "VakuuPlusTestButton";
    ...
    public static void AttachIfMissing(NEndTurnButton instance)
    {
        try
        {
            ...
            MainFile.Logger.Info(
                $"[VakuuPlus] test button attached as child of NEndTurnButton " +
        ...
        {
            MainFile.Logger.Warn($"[VakuuPlus] test button attach failed: {ex.Message}");
        }
    }
    ...
    private static async System.Threading.Tasks.Task OnPressedAsync()
    {
        try
        {
            ...
            if (combatState == null)
            {
                MainFile.Logger.Warn("[VakuuPlus] test button: no active combat state");
                return;
            }
            var player = LocalContext.GetMe(combatState);
            if (player == null)
            {
                MainFile.Logger.Warn("[VakuuPlus] test button: no local player");
                return;
            }

            // One-shot per turn: mark used immediately so the poller can disable the button.
            LastUsedRound = combatState.RoundNumber;
            MainFile.Logger.Info($"[VakuuPlus] test button pressed — running planner (turn {combatState.RoundNumber})");
        ...
        {
            MainFile.Logger.Warn($"[VakuuPlus] test button execution failed: {ex}");
        }
    }
}
