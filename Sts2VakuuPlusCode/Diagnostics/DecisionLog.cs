...
namespace Sts2VakuuPlus.Diagnostics;
...
internal static class DecisionLog
{
    ...
    public static void Dump(Action<string> writer)
    {
        ...
        writer($"[VakuuPlus] === DecisionLog dump ({snapshot.Length} entries) ===");
        foreach (var e in snapshot)
        {
            writer($"[VakuuPlus]   {e.Timestamp:HH:mm:ss} step={e.Step} style={e.Playstyle} "
