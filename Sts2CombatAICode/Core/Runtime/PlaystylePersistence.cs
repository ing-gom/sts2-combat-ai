...
namespace Sts2CombatAI.Runtime;

/// <summary>
/// Persists the current Playstyle selection across game restarts.
/// File: {user_data_dir}/Sts2CombatAI/playstyle.json.
///
...
internal static class PlaystylePersistence
{
    ...
    public static void Install()
    {
        ...
        try
        {
            var dir = Path.Combine(OS.GetUserDataDir(), "Sts2CombatAI");
        ...
        {
            MainFile.Logger.Warn($"[CombatAI] playstyle persistence init failed: {ex.Message}");
        }
    }
        ...
        try
        {
            ...
            if (root.TryGetProperty("playstyle", out var p) && p.ValueKind == JsonValueKind.String)
            {
                ...
                if (!string.IsNullOrEmpty(s) && Enum.TryParse<Playstyle>(s, out var style))
                {
                    ...
                    MainFile.Logger.Info($"[CombatAI] playstyle restored: {style}");
                }
            }
        }
        catch (Exception ex)
        {
            MainFile.Logger.Warn($"[CombatAI] playstyle load failed: {ex.Message}");
        }
    }
    ...
    public static void Save()
    {
        ...
        {
            MainFile.Logger.Warn($"[CombatAI] playstyle save failed: {ex.Message}");
        }
    }
}
