                ...
                if (history != null)
                {
                    foreach (var entry in history.Entries)
                    {
                        ...
                        else if (entry is CardGeneratedEntry cge)
                        {
                            try
                            {
                                // 107 (build 23811903): CardGeneratedEntry.GeneratedByPlayer (bool)
                                // was replaced by Creator (Player?). Creator == player subsumes the
                                // old "generated by player AND owned by player" check.
