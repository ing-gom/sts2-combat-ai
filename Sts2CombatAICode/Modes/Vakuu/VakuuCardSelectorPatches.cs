...
internal static class VakuuCardSelectorPatches
{
        ...
        ref Task<IEnumerable<CardModel>> __result)
    {
        ...
        {
            MainFile.Logger.Warn($"[CombatAI] selector failed, falling back: {ex.Message}");
            return true;
        }
    }
        ...
        ref CardModel? __result)
    {
        ...
        try
        {
            ...
            MainFile.Logger.Info($"[CombatAI] selector(reward): picked={__result?.Id.Entry ?? "null"}");
            return false;
        }
        catch (System.Exception ex)
        {
            MainFile.Logger.Warn($"[CombatAI] reward selector failed, falling back: {ex.Message}");
            return true;
        }
    }
}
